...
function renderSubscriptionScreen() {
  ...
  if (!isPremiumActive()) {
    root.append(createPremiumGateCard({
      onCta: handleUpgrade,
      note: "Уведомления доступны при активном Premium.",
    }));
  } else {
    ...
    remindersHelper.textContent = "Раз в день в Telegram, если нужно поддержать ритм";
    ...
    toggleTrack.setAttribute("aria-label", "Переключить напоминания");
    toggleTrack.disabled = state.reminderSaving || state.busy;
    const toggleThumb = document.createElement("div");
    toggleThumb.className = "reminder-toggle-thumb";
    toggleTrack.append(toggleThumb);
    toggleTrack.addEventListener("click", (event) => {
    ...
    toneHelper.textContent = "Выберите тон сообщений в боте";
